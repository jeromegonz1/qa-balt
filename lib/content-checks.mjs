...
function checkGenericPatterns(baseUrl, pages) {
  ...
  homepageHtml = safeCurl(`${baseUrl}/`, { maxTime: 15 }) || '';
...
function checkAgainstModel(baseUrl, modelUrl, pages) {
  ...
  try {
    ...
    const modelSitemap = safeCurl(`${modelUrl}/sitemap.xml`, { maxTime: 15 });
    if (!modelSitemap) throw new Error('model sitemap fetch failed');
  ...
  if (modelPages.length === 0) {
    try {
      const modelHome = safeCurl(`${modelUrl}/`, { maxTime: 15 });
      if (!modelHome) throw new Error('model home fetch failed');
  ...
  for (const path of modelPages) {
    ...
    const html = safeCurl(`${modelUrl}${path}`, { maxTime: 15 });
    if (!html) continue;
    const mainHtml = extractMainContent(html);
    const text = normalizeText(mainHtml);
    if (text.length > 50) {
