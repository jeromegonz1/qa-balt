...
// ═══════════════════════════════════════
// Protection SSRF — blocage IPs privees
// ═══════════════════════════════════════
...
/**
 * Verifie si une IP est dans une plage privee/reservee
 */
...
/**
 * Valide qu'une URL pointe vers un hote public (pas de SSRF).
 * Bloque localhost, IPs privees, et domaines resolus vers IPs privees.
 *
 * @param {string} urlStr - URL a valider
 * @returns {Promise<URL>} URL parsee si valide
 * @throws {Error} si l'URL est bloquee
 */
export async function validatePublicUrl(urlStr) {
  const parsed = new URL(urlStr);
  const hostname = parsed.hostname;

  // Bloquer localhost explicitement
  if (hostname === 'localhost' || hostname === '::1') {
    throw new Error(`URL bloquee : hostname ${hostname} interdit`);
  }

  // Bloquer si hostname est une IP privee directe
  if (isPrivateIp(hostname)) {
    throw new Error(`URL bloquee : IP privee ${hostname}`);
  }

  // Resoudre DNS pour bloquer les domaines qui pointent vers IPs privees
  try {
    const { address } = await lookup(hostname);
    if (isPrivateIp(address)) {
      throw new Error(`URL bloquee : ${hostname} resout vers IP privee ${address}`);
    }
  } catch (err) {
    if (err.code === 'ENOTFOUND') {
      throw new Error(`URL bloquee : DNS resolution echouee pour ${hostname}`);
    }
    if (err.message.startsWith('URL bloquee')) throw err;
    // Autres erreurs DNS : on laisse passer (le curl echouera de toute facon)
  }
