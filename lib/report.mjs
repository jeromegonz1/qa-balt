...
function groupIssues(allIssues, totalPages) {
  ...
  for (const issue of allIssues) {
    ...
    if (!groups.has(id)) {
      groups.set(id, {
        ...
        pages: new Set(),
        details: [],
        // Phase A : champs enrichis preserves depuis la 1re occurrence
        element: issue.element || null,
        reference: issue.reference || null,
        externalWidget: issue.externalWidget || null,
      });
    }
    ...
    // Si la 1re occurrence n'avait pas l'info mais une autre si — la recuperer
    if (!group.element && issue.element) group.element = issue.element;
    if (!group.reference && issue.reference) group.reference = issue.reference;
    if (!group.externalWidget && issue.externalWidget) group.externalWidget = issue.externalWidget;
  ...
  for (const [, group] of groups) {
    ...
    result.push({
      ...
      pages: [...group.pages],
      // Phase A : champs enrichis (presents si issue source les avait)
      element: group.element || undefined,
      reference: group.reference || undefined,
      externalWidget: group.externalWidget || undefined,
    });
  }
...
export function generateReport(siteUrl, { techIssues, linkIssues = [], pageIssues = [], contentIssues = [], techInfo, visualIssues, a11yIssues = [], perfIssues = [], debuglogIssues = [], pages, screenshotPaths, siteContext = {} }) {
