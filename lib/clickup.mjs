...
export function extractPreprodUrl(task) {
  ...
  const urlField = customFields.find(f =>
    /url.*(?:pr[eé]prod|int[eé]gration)|(?:pr[eé]prod|int[eé]gration).*url|site.*azko|url.*site/i.test(f.name)
  );
...
/**
 * Poste un commentaire texte brut lisible (résumé QA)
 * Pas de markdown — texte brut pour être lisible directement dans ClickUp
 */
export async function postSummaryComment(taskId, { siteName, siteUrl, bloquants, checklistMep, importants, mineurs, total }) {
  const hasBloquants = bloquants > 0;
  const verdict = hasBloquants
    ? `Le site n'est PAS pret pour la prod.`
    : `Le site peut partir en prod (${importants} points a ameliorer).`;

  const text = [
    `QA AUTOMATIQUE — ${siteName.toUpperCase()}`,
    `URL : ${siteUrl}`,
    '',
    'RESULTATS :',
    `  BLOQUANT      : ${bloquants}`,
    `  CHECKLIST MEP : ${checklistMep}`,
    `  IMPORTANT     : ${importants}`,
    `  MINEUR        : ${mineurs}`,
    `  TOTAL         : ${total}`,
    '',
    verdict,
    'Rapport complet en piece jointe (HTML).',
  ].join('\n');
...
/**
 * Uploade un fichier en pièce jointe sur une tâche ClickUp
 * Utilise l'API attachments (multipart/form-data)
 *
 * Node 22 : FormData + Blob natifs — 0 dépendance
 */
export async function uploadAttachment(taskId, filePath, fileName) {
  ...
  if (!res.ok) {
    throw new Error(`ClickUp POST attachment: ${res.status} ${await res.text()}`);
  }
  return res.json();
}
