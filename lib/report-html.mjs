...
`;
  ...
  if (Array.isArray(screenshotPaths) && screenshotPaths.length > 0) {
    ...
    if (thumbs.length > 0) {
      // URLs absolues si QA_BALT_PUBLIC_URL defini, sinon relatives.
      // Les URLs absolues permettent au rapport d'etre telecharge et partage
      // (les images s'affichent depuis le serveur qa-balt). En relatif, ne
      // marche que si le rapport est servi par qa-balt directement.
...
<div class="screenshots-grid">`;
      for (const t of thumbs) {
        const path = `/screenshots/${encodeURIComponent(t.dir)}/${encodeURIComponent(t.file)}`;
