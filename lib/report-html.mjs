...
/* ─── Screenshot element annote (sprint 4) ────────────────────────── */
.issue-screenshot-link { display: block; margin-top: 8px; text-decoration: none; }
.issue-screenshot { max-width: 100%; max-height: 320px; border: 2px solid #e11; border-radius: 4px; display: block; box-shadow: 0 2px 8px rgba(225,17,17,0.2); background: #fafafa; }
.issue-screenshot-link:hover .issue-screenshot { box-shadow: 0 4px 12px rgba(225,17,17,0.4); transform: scale(1.01); transition: transform 0.15s, box-shadow 0.15s; }
@media print { .issue-screenshot { max-height: 200px; box-shadow: none; } }
...
function renderIssueExtras(issue, siteUrl) {
  ...
  // ─ Element structure (selector + html snippet + location + screenshot) ─
  if (issue.element && (issue.element.selector || issue.element.html || issue.element.screenshot)) {
    const { selector, html: snippet, location, screenshot } = issue.element;
    const liveUrl = selector
      ? `${siteUrl}/?qa_highlight=${encodeURIComponent(selector)}`
      : null;

    // Sprint 4 : screenshot annote (outline rouge) — URL absolue si possible
    let screenshotHtml = '';
    if (screenshot) {
      const publicUrl = (process.env.QA_BALT_PUBLIC_URL || '').replace(/\/+$/, '');
      // Le path absolu filesystem (ex: /home/ubuntu/qa-balt/screenshots/site-date/issues/issue-x.png)
      // est transforme en URL /screenshots/{dir}/issues/{file}
      const parts2 = screenshot.split('/').filter(Boolean);
      const file = parts2.pop();
      const subdir = parts2.pop(); // "issues"
      const siteDir = parts2.pop(); // "site-date"
      if (file && subdir === 'issues' && siteDir && /\.png$/i.test(file)) {
        const path = `/screenshots/${encodeURIComponent(siteDir)}/${encodeURIComponent(subdir)}/${encodeURIComponent(file)}`;
        const url = publicUrl ? `${publicUrl}${path}` : path;
        screenshotHtml = `<a href="${esc(url)}" target="_blank" rel="noopener" class="issue-screenshot-link">`
          + `<img src="${esc(url)}" loading="lazy" alt="Screenshot annoté de l'élément" class="issue-screenshot">`
          + `</a>`;
      }
    }
...
`;
  ...
  // Note : galerie screenshots de pages retiree (sprint 4-pre rollback) —
  // remplacee par les screenshots ELEMENT annotes dans chaque carte issue
  // (sprint 4, value reelle pour l'integrateur).
