...
import { captureElement, buildIssueScreenshotName } from './element-capture.mjs';
import { resolve } from 'path';

export async function runA11yChecks(baseUrl, pages, screenshotsDir = null) {
  const issues = [];
    ...
    try {
      ...
      for (const violation of results.violations) {
        ...
        if (!entry.element && violation.nodes[0]) {
          ...
          // Sprint 4 : capture screenshot annote (outline rouge) si screenshotsDir fourni
          if (screenshotsDir && firstSelector) {
            try {
              const filename = buildIssueScreenshotName(entry.id);
              const outputPath = resolve(screenshotsDir, 'issues', filename);
              const saved = await captureElement(page, firstSelector, outputPath);
              if (saved) entry.element.screenshot = saved;
            } catch { /* fail silent — element capture est best-effort */ }
          }
        }
