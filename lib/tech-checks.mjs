...
import { findMapReference } from './geo-extraction.mjs';
import { buildElementInfo } from './html-ancestry.mjs';
import { classifyLocation } from './element-location.mjs';
import { BLACKLISTED_EMAILS, BLACKLISTED_NAMES, DEFAULT_CMS_PAGES, DEFAULT_ADDRESSES, DEFAULT_404_SIGNATURES, ACCEPTED_FORMATS, SELECTORS, BALT_DEFAULT_MAPS_URLS } from './config.mjs';

export function runTechChecks(baseUrl, pages = []) {
  const issues = [];
  const info = {};
  ...
  // --- Schema.org GPS 0,0 (catches "0", 0, 0.0, 0.00, "0.0", case-insensitive) ---
  ...
  if (latMatch && lngMatch) {
    ...
    if (Number.isFinite(lat) && Number.isFinite(lng) && lat === 0 && lng === 0) {
      // Cross-check : si Maps configure ailleurs sur le site, fournir une suggestion adaptee
      const ref = findMapReference(baseUrl, pages);
      let suggestion = '';
      if (ref?.coords) {
        suggestion = ` 💡 Suggestion : l'iframe Maps de \`${ref.page}\` pointe sur lat=${ref.coords.lat}, lng=${ref.coords.lng} — Schema.org devrait utiliser ces valeurs.`;
      } else if (ref?.address) {
        suggestion = ` 💡 Suggestion : l'iframe Maps de \`${ref.page}\` utilise l'adresse texte « ${ref.address} » (Embed API v1) — geocoder cette adresse et reporter les coords dans Schema.org.`;
      } else if (ref?.shortLink) {
        suggestion = ` 💡 Suggestion : un short URL Google Maps est utilise sur \`${ref.page}\` (${ref.shortLink}) — ouvrir ce lien pour recuperer les coords (lat/lng dans l'URL complete) et les reporter dans Schema.org JSON-LD.`;
      }
      issues.push({
        severity: 'BLOQUANT',
        id: 'SCHEMA_GPS_ZERO',
        title: 'Coordonnées GPS Schema.org à 0,0',
        detail: `latitude: ${latMatch[1]}, longitude: ${lngMatch[1]} → océan Atlantique. A corriger.${suggestion}`,
      });
    }
  }
  ...
  // --- URLs Google Maps "default" des demos BALT (sprint user feedback) ---
  // Le « lien nous localiser » du footer / page contact pointe par defaut
  // sur une URL Maps fictive Septeo. Si l'integrateur oublie de la remplacer,
  // tous les clients se retrouvent sur le meme point — impact direct.
  for (const entry of BALT_DEFAULT_MAPS_URLS) {
    // Match URL avec ou sans https:// prefix
    const urlEscaped = entry.url.replace(/[.+?^${}()|[\]\\]/g, '\\$&');
    const re = new RegExp(`(?:https?:\\/\\/)?${urlEscaped}`, 'i');
    const m = html.match(re);
    if (m) {
      // Compter le nombre d'occurrences (footer + contact = 2 typiquement)
      const allMatches = html.match(new RegExp(re.source, 'gi'));
      const occurrences = allMatches ? allMatches.length : 1;
      const elInfo = buildElementInfo(html, m.index, m[0], 'a', classifyLocation);
      issues.push({
        severity: 'BLOQUANT',
        id: 'MAP_BALT_DEFAULT_LINK',
        title: 'Lien "Nous localiser" pointe sur la démo BALT',
        detail: `URL Maps fictive du modèle BALT toujours présente : ${entry.url} (${occurrences} occurrence${occurrences > 1 ? 's' : ''} — typiquement footer + contact). Source : ${entry.source}. À remplacer par le vrai lien Google Maps du client.`,
        element: elInfo,
      });
      break; // 1 issue suffit, l'integrateur fixera toutes les occurrences
    }
  }

  // --- Footer email vide ---
  const footerMailRe = /<div[^>]*class="footer__mail"[^>]*>\s*Mail\s*:\s*<\/div/i;
  const footerMailMatch = html.match(footerMailRe);
  if (footerMailMatch || /class="footer__mail">\s*Mail\s*:\s*$/m.test(html)) {
    const elInfo = footerMailMatch
      ? buildElementInfo(html, footerMailMatch.index, footerMailMatch[0], 'div', classifyLocation)
      : { selector: '.footer__mail', html: 'Mail : (vide)', location: 'footer' };
    issues.push({
      severity: 'IMPORTANT',
  ...
  const genericAltsIter = [...html.matchAll(/<img[^>]*alt="(?:Diaporama|Image|Photo|img)"[^>]*>/gi)];
  if (genericAltsIter.length > 2) {
    // Sprint 3 : enrichir avec selector/location du 1er match
    const first = genericAltsIter[0];
    const elInfo = buildElementInfo(html, first.index, first[0], 'img', classifyLocation);
    issues.push({
      severity: 'IMPORTANT',
      id: 'ALT_GENERIC',
      title: `${genericAltsIter.length} images avec alt générique`,
  ...
  // --- Images sans alt ---
  const noAltIter = [...html.matchAll(/<img(?![^>]*alt=)[^>]*>/gi)];
  if (noAltIter.length > 0) {
    const first = noAltIter[0];
    const elInfo = buildElementInfo(html, first.index, first[0], 'img', classifyLocation);
    const noAlt = noAltIter.map(m => m[0]);
    issues.push({
      severity: 'IMPORTANT',
      id: 'ALT_MISSING',
  ...
  // ═══════════════════════════════════════
  // Retour terrain : certains sites integrent la politique dans un accordeon
  // de /mentions-legales.htm au lieu d'une page dediee. On verifie d'abord
  // que la page dediee existe ; si 404, on cherche dans les mentions legales
  // un signal "politique / confidentialite / RGPD" avant de remonter.
  try {
    ...
    if (pdcStatus === '404' || pdcStatus === '000') {
      ...
      const isReal404 = /introuvable|not found|404/i.test(pdcHtml) || pdcStatus === '404';

      if (isReal404) {
        // Fallback : la politique peut etre integree dans /mentions-legales.htm
        // (cas typique : accordeon avec sections). On cherche un signal explicite.
        const mlPaths = ['/mentions-legales.htm', '/mentions-legales', '/legal.htm'];
        let foundInMentions = null;
        for (const path of mlPaths) {
          const mlHtml = safeCurl(`${baseUrl}${path}`, { maxTime: 10 });
          if (!mlHtml) continue;
          // Cherche les termes RGPD courants dans le HTML des mentions legales
          if (/politique\s+de\s+confidentialit[eé]|donn[ée]es\s+personnelles|RGPD|GDPR/i.test(mlHtml)) {
            foundInMentions = path;
            break;
          }
        }

        if (foundInMentions) {
          // Politique presente mais sous forme d'accordeon — downgrade en CHECKLIST_MEP
          issues.push({
            severity: 'CHECKLIST_MEP',
            id: 'PRIVACY_IN_MENTIONS',
            title: 'Politique de confidentialité intégrée dans les mentions légales',
            detail: `Pas de page /politique-de-confidentialite.htm dédiée, mais une section dans \`${foundInMentions}\`. Vérifier que c'est volontaire (accordéon valide RGPD) et que le footer pointe correctement.`,
          });
        } else {
          // Vraiment introuvable nulle part : bloquant
          issues.push({
            severity: 'BLOQUANT',
            id: 'PRIVACY_404',
            title: 'Page Politique de confidentialité = 404',
            detail: 'Obligatoire RGPD (formulaire de contact + cookies présents). Recherchée dans /politique-de-confidentialite.htm + /mentions-legales.htm — aucun signal RGPD trouvé.',
          });
        }
      }
    }
