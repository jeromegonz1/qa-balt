...
export function runTechChecks(baseUrl) {
  ...
  try {
    ...
    const response404 = execSync(
      `curl -sL -k --max-time 10 "${test404Url}" 2>/dev/null`,
      { encoding: 'utf-8', timeout: 15000 }
    );
    const status404 = execSync(
      `curl -sL -k --max-time 10 "${test404Url}" -o /dev/null -w "%{http_code}" 2>/dev/null`,
      { encoding: 'utf-8', timeout: 15000 }
    ).trim();
