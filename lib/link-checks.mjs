...
import { DIRTY_URL_PATTERNS, SOCIAL_DOMAINS, SOCIAL_BLACKLIST_PATTERNS, BOOKING_DEMO_PATTERNS } from './config.mjs';
...
export function runLinkChecks(baseUrl, pages, siteContext = {}) {
  ...
  for (const p of pages) {
    ...
    if (!html) continue; // Page inaccessible, déjà reporté par le crawler

    const links = extractLinks(html, p.name);
    linksByPage[p.name] = { links, html };
  ...
  for (const [path, data] of internalLinks) {
    try {
      ...
      const result = safeCurl(`${baseUrl}${path}`, {
        maxTime: 8, headOnly: true,
  ...
  for (const [url, data] of externalLinks) {
    try {
      const status = (safeCurl(url, {
        maxTime: 8, headOnly: true, outputDev: 'null', writeOut: '%{http_code}',
      }) || '0').trim();
  ...
  for (const sl of socialLinks) {
    ...
    try {
      ...
      const curlResult = safeCurl(sl.href, {
        maxTime: 10, followRedirects: true,
        writeOut: '\n%{http_code}',
        maxBuffer: 512 * 1024,
      });
      if (!curlResult) continue;
  ...
  // ═══════════════════════════════════════
  // COUCHE 6 : Liens réservation démo (camping/hôtel)
  //   Config : BOOKING_DEMO_PATTERNS dans config.mjs
  //   Scanne tous les liens + iframes pour détecter les moteurs de resa
  //   pointant vers un compte démo (ex: Thelis demosalons)
  // ═══════════════════════════════════════
  for (const [href, { pages: linkPages }] of uniqueLinks) {
    for (const bp of BOOKING_DEMO_PATTERNS) {
      if (bp.regex.test(href)) {
        const pageCount = linkPages.size;
        const scope = pageCount >= Math.max(2, Math.floor(pages.length * 0.4)) ? 'template' : null;
        const pageLabel = scope ? `template — ${pageCount} pages` : [...linkPages].slice(0, 3).join(', ');
        ...
        issues.push({
          severity: 'IMPORTANT',
          id: 'BOOKING_DEMO_LINK',
          page: [...linkPages][0],
          title: `Lien réservation pointe vers la démo ${bp.engine}`,
          detail: `${bp.label} — ${href.substring(0, 120)} (${pageLabel}). Remplacer par le compte client.`,
        });
        break;
      }
    }
  ...
  // Scanner aussi les iframes de réservation dans le HTML brut des pages
  // Reutilise le HTML deja fetche en couche 1 (pas de double-fetch)
  for (const p of pages) {
    if (p.status && p.status >= 400) continue;
    const pageData = linksByPage[p.name];
    if (!pageData?.html) continue;

    // Chercher les iframes et scripts de moteur de resa
    try {
      ...
      for (const src of allSrcs) {
        for (const bp of BOOKING_DEMO_PATTERNS) {
          if (bp.regex.test(src)) {
            issues.push({
              severity: 'IMPORTANT',
              id: 'BOOKING_DEMO_IFRAME',
              page: p.name,
              title: `Iframe/script réservation pointe vers la démo ${bp.engine}`,
              detail: `${bp.label} — ${src.substring(0, 120)} (${p.name}). Remplacer par le compte client.`,
            });
            break;
          }
        }
      }
    } catch (e) { /* page déjà fetchée plus haut, on ignore les erreurs */ }
  }
