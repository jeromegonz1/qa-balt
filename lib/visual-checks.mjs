...
import { VISUAL_THRESHOLDS } from './config.mjs';
  ...
  for (const p of pages) {
    ...
    try {
      ...
      if (brokenImages.length > 0) {
        ...
        // Sprint 1.d : si la grande majorite des images cassees vient du lazy
        // loader AZKO (src vide), on tag specifiquement. C'est un bug CMS connu
        // hors de portee de l'integrateur (retour Julien : « Nous n'avons pas
        // la main sur cette fonctionnalite »). On garde BLOQUANT si massif
        // (>=10 sur la page) car ca degrade reellement l'experience, mais on
        // ajoute une note explicite dans le detail.
        const isLazyLoaderBug = noSrc.length > 0 && noSrc.length >= unique.length * 2;
        const lazyNote = isLazyLoaderBug
          ? ' ⚠️ Bug CMS AZKO connu (lazy loader GSAP) — hors de portée de l\'intégrateur. À remonter à l\'équipe dev AZKO.'
          : '';

        // Severite : seuil different selon que c'est un bug CMS ou non.
        // - Bug lazy load CMS : tolerance plus haute (>=10 = BLOQUANT) car
        //   l'integrateur ne peut pas fixer, on signale juste l'ampleur.
        // - Cas normal : >=5 = BLOQUANT (vrai probleme actionable).
        const severity = isLazyLoaderBug
          ? (total >= 10 ? 'BLOQUANT' : 'IMPORTANT')
          : (total >= 5 ? 'BLOQUANT' : 'IMPORTANT');

        issues.push({
          severity,
          id: isLazyLoaderBug ? 'IMG_BROKEN_LAZY_CMS' : 'IMG_BROKEN',
          page: p.name,
          title: `${total} image(s) cassée(s)${isLazyLoaderBug ? ' (lazy load CMS)' : ''}`,
          detail: details.join(' | ') + lazyNote,
        });
      }
      ...
      const smallFonts = await mp.evaluate(() => {
        ...
        for (const el of nodes) {
          ...
          if (size < VISUAL_THRESHOLDS.fontMinPx && el.textContent.trim().length > 5) {
            const tag = el.tagName.toLowerCase();
            const cls = el.className?.toString().split(' ')[0] || '';
      ...
      if (headerInfo && headerInfo.pct > VISUAL_THRESHOLDS.headerMaxPercent) {
        issues.push({
      ...
      if (heroInfo && heroInfo.pct > VISUAL_THRESHOLDS.heroMaxPercent) {
        issues.push({
      ...
      const distortedImages = await mp.evaluate(() => {
        ...
        for (const img of document.querySelectorAll('img')) {
          ...
          if (diff > VISUAL_THRESHOLDS.imgDistortionThreshold) {
